...
# @unittest.skip("Skipping main.py export_iso3166_2 tests.")
...
#1.)
        export_iso3166_2(alpha_codes=test_alpha_dk, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, 
                         export_csv=1, export_xml=1, history=False) #Denmark
        
        #open exported json, csv and xml
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_{test_alpha_dk}.json'))  as output_json:
        ...
        self.assertEqual(list(test_iso3166_2_dk_json["DK"].keys()), ['DK-81', 'DK-82', 'DK-83', 'DK-84', 'DK-85'], 
            f"Expected subdivision codes do not match. Expected: ['DK-81', 'DK-82', 'DK-83', 'DK-84', 'DK-85'], Got: {list(test_iso3166_2_dk_json['DK'].keys())}")   
        ...
        self.assertEqual(list(test_iso3166_2_dk_csv.columns), self.correct_default_output_columns[1:], 
            f"Expected CSV columns do not match. Expected: {self.correct_default_output_columns[1:]}, Got: {list(test_iso3166_2_dk_csv.columns)}.")
        ...
        for top_level in test_iso3166_2_dk_xml.getroot():
            for second_level in top_level:
                observed = set([child.tag for child in second_level])
                expected = set(self.correct_default_output_attributes)
                self.assertEqual(observed, expected, 
                                 f"Expected second-level XML attributes do not match. Expected: {expected}, Got: {observed}.")
        ...
        test_iso3166_2_dk_json_dk_81_expected = {"name": "Nordjylland", "localOtherName": "North Denmark (eng), North Jutland Region (eng)", "parentCode": None, 
            "type": "Region", "flag": "https://raw.githubusercontent.com/amckenna41/iso3166-flags/main/iso3166-2-flags/DK/DK-81.svg", "latLng": [56.8153, 9.7273]}
        ...
        test_iso3166_2_dk_json_dk_82_expected = {"name": "Midtjylland", "localOtherName": "Central Denmark (eng), Central Jutland Region (eng), Mid-Jutland (eng)", 
            "parentCode": None, "type": "Region", "flag": "https://raw.githubusercontent.com/amckenna41/iso3166-flags/main/iso3166-2-flags/DK/DK-82.svg", "latLng": [56.2356, 9.2346]}
...
#2.)    
        export_iso3166_2(alpha_codes=test_alpha_fi, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, 
                         export_xml=True, history=False) #Finland
        
        #open exported json, csv and xml
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_{test_alpha_fi}.json'))  as output_json:
        ...
        self.assertEqual(list(test_iso3166_2_fi_json["FI"].keys()), ["FI-01", "FI-02", "FI-03", "FI-04", "FI-05", "FI-06", "FI-07", "FI-08", "FI-09", 
            "FI-10", "FI-11", "FI-12", "FI-13", "FI-14", "FI-15", "FI-16", "FI-17", "FI-18", "FI-19"], 
                f"Expected subdivision codes do not match. Expected: FI-01 to FI-19, Got: {list(test_iso3166_2_fi_json['FI'].keys())}")
        ...
        test_iso3166_2_fi_json_fi_02_expected = {"name": "Etelä-Karjala", "localOtherName": "Södra Karelen (swe), South Karelia (eng)", "parentCode": None, 
            "type": "Region", "flag": "https://raw.githubusercontent.com/amckenna41/iso3166-flags/main/iso3166-2-flags/FI/FI-02.png", "latLng": [61.2667, 28.4055]}
        ...
        test_iso3166_2_fi_json_fi_17_expected = {"name": "Satakunta", "localOtherName": "Satakunta (swe), Satakunta (eng)", "parentCode": None, 
            "type": "Region", "flag": "https://raw.githubusercontent.com/amckenna41/iso3166-flags/main/iso3166-2-flags/FI/FI-17.svg", "latLng": [61.5934, 21.7825]}
...
#3.)
        export_iso3166_2(test_alpha_nz, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, 
                         export_xml=True, history=False) #New Zealand

        #open exported json, csv and xml
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_NZ.json')) as output_json:
            test_iso3166_2_nz_json = json.load(output_json)
        test_iso3166_2_nz_csv = pd.read_csv(os.path.join(self.test_output_dir, f'{self.test_output_filename}_NZ.csv'))
        test_iso3166_2_nz_xml = ET.parse(os.path.join(self.test_output_dir, f'{self.test_output_filename}_NZ.xml'))
        ...
        self.assertEqual(list(test_iso3166_2_nz_json['NZ'].keys()), ['NZ-AUK', 'NZ-BOP', 'NZ-CAN', 'NZ-CIT', 'NZ-GIS', 'NZ-HKB', 'NZ-MBH', 'NZ-MWT', 'NZ-NSN', 'NZ-NTL', 'NZ-OTA', 'NZ-STL', 'NZ-TAS', 'NZ-TKI', 'NZ-WGN', 'NZ-WKO', 'NZ-WTC'], 
            f"Expected subdivision codes do not match. Expected: ['NZ-AUK', 'NZ-BOP', ..., 'NZ-WTC'], Got: {list(test_iso3166_2_nz_json['NZ'].keys())}")
        ...
        self.assertEqual(list(test_iso3166_2_nz_csv.columns), self.correct_default_output_columns[1:], f"Expected CSV columns do not match. Expected: {self.correct_default_output_columns[1:]}, Got: {list(test_iso3166_2_nz_csv.columns)}.")
        ...
        test_iso3166_2_nz_json_nz_ota_expected = {"name": "Otago", "localOtherName": "Ō Tākou (mri)", 
            "parentCode": None, "type": "Region", "flag": "https://raw.githubusercontent.com/amckenna41/iso3166-flags/main/iso3166-2-flags/NZ/NZ-OTA.svg", "latLng": [-45.3975, 170.1779]}
        ...
        test_iso3166_2_nz_json_nz_tki_expected = {"name": "Taranaki", "localOtherName": "Taranaki (mri)", 
            "parentCode": None, "type": "Region", "flag": None, "latLng": [-39.3848, 174.1974]}
...
#4.)
        export_iso3166_2(alpha_codes=test_alpha_kg_mg_nr, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, 
                         export_xml=True, history=False) #Kyrgyzstan, Madagascar, Nauru

        #open exported json, csv and xml
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_KG,MG,NR.json')) as output_json:
            test_iso3166_2_kg_mg_nr_json = json.load(output_json)
        test_iso3166_2_kg_mg_nr_csv = pd.read_csv(os.path.join(self.test_output_dir, f'{self.test_output_filename}_KG,MG,NR.csv'))
        test_iso3166_2_kg_mg_nr_xml = ET.parse(os.path.join(self.test_output_dir, f'{self.test_output_filename}_KG,MG,NR.xml'))
        ...
        self.assertEqual(list(test_iso3166_2_kg_mg_nr_json["KG"].keys()), ['KG-B', 'KG-C', 'KG-GB', 'KG-GO', 'KG-J', 'KG-N', 'KG-O', 'KG-T', 'KG-Y'], 
            f"Expected subdivision codes do not match. Expected: ['KG-B', 'KG-C', ..., 'KG-Y'], Got: {list(test_iso3166_2_kg_mg_nr_json['KG'].keys())}")
        self.assertEqual(list(test_iso3166_2_kg_mg_nr_json["MG"].keys()), ["MG-A", "MG-D", "MG-F", "MG-M", "MG-T", "MG-U"], 
            f"Expected subdivision codes do not match. Expected: ['MG-A', 'MG-D', ..., 'MG-U'], Got: {list(test_iso3166_2_kg_mg_nr_json['MG'].keys())}")
        self.assertEqual(list(test_iso3166_2_kg_mg_nr_json["NR"].keys()), ['NR-01', 'NR-02', 'NR-03', 'NR-04', 'NR-05', 'NR-06', 'NR-07', 'NR-08', 'NR-09', 'NR-10', 
            'NR-11', 'NR-12', 'NR-13', 'NR-14'], f"Expected subdivision codes do not match. Expected: ['NR-01', ..., 'NR-14'], Got: {list(test_iso3166_2_kg_mg_nr_json['NR'].keys())}")
        ...
        self.assertEqual(list(test_iso3166_2_kg_mg_nr_csv.columns), self.correct_default_output_columns, f"Expected CSV columns do not match. Expected: {self.correct_default_output_columns}, Got: {list(test_iso3166_2_kg_mg_nr_csv.columns)}.")
        ...
        test_iso3166_2_kg_json_kg_gb_expected = {"name": "Bishkek Shaary", "localOtherName": "Бишкек шаары (kir), Город Бишкек (rus), Gorod Bishkek (rus), Gorod Biškek (rus), Bishkek (eng)", 
            "parentCode": None, "type": "City", "flag": "https://raw.githubusercontent.com/amckenna41/iso3166-flags/main/iso3166-2-flags/KG/KG-GB.svg", "latLng": [42.8761, 74.6037]}
        ...
        #MG-D - Antsiranana
        test_iso3166_2_mg_sa_json_mg_d_expected = {"name": "Antsiranana", "localOtherName": None, "parentCode": None, "type": "Province", "flag": None, "latLng": [-13.723, 49.1802]}        
        ...
        #NR-02 - Anabar
        test_iso3166_2_nr_json_nr_02_expected = {"name": "Anabar", "localOtherName": None, "parentCode": None, "type": "District", "flag": None, "latLng": [-0.5135, 166.9512]}
...
#1.)
        export_iso3166_2(alpha_codes=test_alpha_gt, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1,
                         rest_countries_keys="continents,subregion", history=False) #Guatemala - including additional continent and region attributes
        expected_attribute_list = self.correct_default_output_attributes + ["continents", "subregion"]

        #open exported json & csv
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_GT.json')) as output_json:
        ...
        self.assertEqual(list(test_iso3166_2_gt_json["GT"].keys()), ['GT-01', 'GT-02', 'GT-03', 'GT-04', 'GT-05', 'GT-06', 'GT-07', 'GT-08', 'GT-09', 'GT-10', 'GT-11', 
            ...
            f"Expected subdivision codes do not match. Expected: ['GT-01' through 'GT-22'], Got: {list(test_iso3166_2_gt_json['GT'].keys())}.")   
...
#2.)
        export_iso3166_2(alpha_codes=test_alpha_ir, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1,
                         rest_countries_keys="languages,startOfWeek,region", history=False) #Iran - including additional languages, startOfWeek and region attributes
        ...
        self.assertEqual(list(test_iso3166_2_ir_json["IR"].keys()), ['IR-00', 'IR-01', 'IR-02', 'IR-03', 'IR-04', 'IR-05', 'IR-06', 'IR-07', 'IR-08', 'IR-09', 'IR-10', 
            ...
            f"Expected subdivision codes do not match. Expected: ['IR-00' through 'IR-30'], Got: {list(test_iso3166_2_ir_json['IR'].keys())}.")   
...
#3.)
        export_iso3166_2(alpha_codes=test_alpha_mg_sa, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1,
                         rest_countries_keys="idd,currencies,postalCode", history=False) #Madagascar, Saudi Arabia - including additional idd, currencies and postalCode attributes
        ...
        self.assertEqual(list(test_iso3166_2_mg_sa_json["MG"].keys()), ['MG-A', 'MG-D', 'MG-F', 'MG-M', 'MG-T', 'MG-U'], 
            f"Expected subdivision codes do not match. Expected: ['MG-A', 'MG-D', 'MG-F', 'MG-M', 'MG-T', 'MG-U'], Got: {list(test_iso3166_2_mg_sa_json['MG'].keys())}.")   
        self.assertEqual(list(test_iso3166_2_mg_sa_json["SA"].keys()), ['SA-01', 'SA-02', 'SA-03', 'SA-04', 'SA-05', 'SA-06', 'SA-07', 'SA-08', 'SA-09', 'SA-10', 'SA-11', 'SA-12', 'SA-14'], 
            f"Expected subdivision codes do not match. Expected: ['SA-01' through 'SA-14' except SA-13], Got: {list(test_iso3166_2_mg_sa_json['SA'].keys())}.")   
...
#4.)
        export_iso3166_2(alpha_codes=test_alpha_tz, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1,
                         rest_countries_keys="*", history=False) #Tanzania - getting all keys via * wildcard
        ...
        self.assertEqual(list(test_iso3166_2_tz_json["TZ"].keys()),['TZ-01', 'TZ-02', 'TZ-03', 'TZ-04', 'TZ-05', 'TZ-06', 'TZ-07', 'TZ-08', 'TZ-09', 'TZ-10', 
            ...
            f"Expected subdivision codes do not match. Expected: ['TZ-01' through 'TZ-31'], Got: {list(test_iso3166_2_tz_json['TZ'].keys())}.")   
...
#1.)
        export_iso3166_2(alpha_codes=test_alpha_ge, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, 
                      filter_attributes="name,latLng,type", history=False) #Georgia - filtering only the name, latLng and type default attributes 
        ...
        self.assertEqual(list(test_iso3166_2_ge_json["GE"].keys()), ['GE-AB', 'GE-AJ', 'GE-GU', 'GE-IM', 'GE-KA', 'GE-KK', 'GE-MM', 'GE-RL', 'GE-SJ', 'GE-SK', 'GE-SZ', 'GE-TB'], 
            f"Expected subdivision codes do not match. Expected: ['GE-AB', 'GE-AJ', ..., 'GE-TB'], Got: {list(test_iso3166_2_ge_json['GE'].keys())}.")   
...
#2.) 
        export_iso3166_2(alpha_codes=test_alpha_gw, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1,
                      filter_attributes=["name", "localOtherName"], history=False) #Guinea-Bissau - filtering the localOtherName and name default attributes 
        ...
        self.assertEqual(list(test_iso3166_2_gw_json["GW"].keys()), ['GW-BA', 'GW-BL', 'GW-BM', 'GW-BS', 'GW-CA', 'GW-GA', 'GW-L', 'GW-N', 'GW-OI', 'GW-QU', 'GW-S', 'GW-TO'], 
            f"Expected subdivision codes do not match. Expected: ['GW-BA', 'GW-BL', ..., 'GW-TO'], Got: {list(test_iso3166_2_gw_json['GW'].keys())}.")   
...
#3.)
        export_iso3166_2(alpha_codes=test_alpha_ki, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1,
                         filter_attributes="parentCode, flag", history=False) #Kiribati - filtering the parentCode and flag default attributes 
        ...
        self.assertEqual(list(test_iso3166_2_ki_json["KI"].keys()), ['KI-G', 'KI-L', 'KI-P'], 
            f"Expected subdivision codes do not match. Expected: ['KI-G', 'KI-L', 'KI-P'], Got: {list(test_iso3166_2_ki_json['KI'].keys())}.")   
...
#4.)
        with (self.assertRaises(ValueError)):
            export_iso3166_2(export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, filter_attributes=test_exclude_keys_error1) #ZZ
            export_iso3166_2(export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, filter_attributes=test_exclude_keys_error2) #ABCDEF
            export_iso3166_2(export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, filter_attributes=test_exclude_keys_error3) #1234
...
#1.)
        export_iso3166_2(alpha_codes_range=test_alpha_at_be, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1) #AT-BE

        #open exported json & csv
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_{test_alpha_at_be}.json')) as output_json:
...
#2.)
        export_iso3166_2(alpha_codes_range=test_alpha_gd_gm, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1) #GD-GM
...
#3.)
        export_iso3166_2(alpha_codes_range=test_alpha_va_vu, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1) #VC-VU
...
#4.)
        export_iso3166_2(alpha_codes_range=test_alpha_ye, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1) #YE
...
#5.)
        with self.assertRaises(ValueError):
            export_iso3166_2(alpha_codes_range=test_alpha_aa_ad, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1) #AA-AD
            export_iso3166_2(alpha_codes_range=test_alpha_xm_xx, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1) #XM-XX
#6.)
        with self.assertRaises(TypeError):
            export_iso3166_2(alpha_codes_range=123, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1) #123
            export_iso3166_2(alpha_codes_range=10.05, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1) #10.4
            export_iso3166_2(alpha_codes_range=False, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1) #False
...
#1.)
        export_iso3166_2(alpha_codes=test_alpha_ae, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, history=True) #UAE 

        #open exported json & csv
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_AE.json')) as output_json:
            test_iso3166_2_ae_json = json.load(output_json)

        #AE-AJ - 'Ajmān
        expected_ae_aj_history = [{'Change': 'Change of spelling of AE-AJ, AE-RK; addition of local variation of AE-FU, AE-RK, AE-UQ; update List Source.', 'Description of Change': None, 'Date Issued': '2015-11-27', 'Source': 'Online Browsing Platform (OBP) - https://www.iso.org/obp/ui/#iso:code:3166:AE.'}]
        ...
        #AE-AZ - Abū Z̧aby
        expected_ae_az_history = [{'Change': 'Error correction: Spelling correction in AE-AZ.', 'Description of Change': None, 'Date Issued': '2002-08-20', 'Source': 'Newsletter I-3 - https://web.archive.org/web/20081218103236/http://www.iso.org/iso/iso_3166-2_newsletter_i-3_en.pdf.'}]
        ...
        expected_ae_rk_history = [{'Change': 'Change of spelling of AE-AJ, AE-RK; addition of local variation of AE-FU, AE-RK, AE-UQ; update List Source.', 'Description of Change': None, 'Date Issued': '2015-11-27', 'Source': 'Online Browsing Platform (OBP) - https://www.iso.org/obp/ui/#iso:code:3166:AE.'}, 
                                  {'Change': 'Spelling correction in AE-RK.', 'Description of Change': None, 'Date Issued': '2002-05-21', 'Source': 'Newsletter I-2 - https://web.archive.org/web/20081218103157/http://www.iso.org/iso/iso_3166-2_newsletter_i-2_en.pdf.'}]
...
#2.)
        export_iso3166_2(alpha_codes=test_alpha_it, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, history=True) #Italy 

        #open exported json & csv
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_IT.json')) as output_json:
            test_iso3166_2_it_json = json.load(output_json)

        #IT-BT - Barletta-Andria-Trani
        expected_it_bt_history = [{'Change': 'Subdivisions added: IT-BT Barletta-Andria-Trani. IT-FM Fermo. IT-MB Monza e Brianza. Codes: IT-FO Forlì -> IT-FC Forlì-Cesena.', 
        ...
        #IT-FM -Fermo
        expected_it_fm_history = [{'Change': 'Subdivisions added: IT-BT Barletta-Andria-Trani. IT-FM Fermo. IT-MB Monza e Brianza. Codes: IT-FO Forlì -> IT-FC Forlì-Cesena.', 
                                   'Description of Change': 'Addition of the country code prefix as the first code element, administrative update.', 
                                   'Date Issued': '2010-02-03 (corrected 2010-02-19)', 
                                   'Source': 'Newsletter II-1 - https://www.iso.org/files/live/sites/isoorg/files/archive/pdf/en/iso_3166-2_newsletter_ii-1_corrected_2010-02-19.pdf.'}]
        ...
        #IT-SU - Sud Sardegna
        expected_it_su_history = [{'Change': 'Code changes: IT-SD -> IT-SU Sud Sardegna. Subdivisions added: IT-GO Gorizia. IT-PN Pordenone. IT-TS Trieste. IT-UD Udine.', 
...
#3.)
        export_iso3166_2(alpha_codes=test_alpha_si, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, history=True) #Slovenia 

        #open exported json & csv
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_SI.json')) as output_json:
            test_iso3166_2_si_json = json.load(output_json)

        #SI-044 - Kanal ob Soči	
        expected_si_044_history = [{'Change': 'Change of spelling of SI-044, SI-197; Addition of category urban municipality; Change of category name from municipality to urban municipality for SI-011, SI-050, SI-052, SI-054, SI-061, SI-070, SI-080, SI-084, SI-085, SI-096, SI-112, SI-133; Update List Source.', 
                                   'Description of Change': None, 
                                   'Date Issued': '2022-11-29', 
                                   'Source': 'Online Browsing Platform (OBP) - https://www.iso.org/obp/ui/#iso:code:3166:SI.'}]
        ...
        #SI-065 - Loška dolina	
        expected_si_065_history = [{'Change': 'Correction of spelling for SI-065, SI-116, SI-169, SI-182, SI-204, SI-210; Deletion of asterisk from SI-212; Update List Source.', 
                                   'Description of Change': None, 
                                   'Date Issued': '2020-11-24', 
                                   'Source': 'Online Browsing Platform (OBP) - https://www.iso.org/obp/ui/#iso:code:3166:SI.'}]
        ...
        #SI-212 - Mirna
        expected_si_212_history = [{'Change': 'Correction of spelling for SI-065, SI-116, SI-169, SI-182, SI-204, SI-210; Deletion of asterisk from SI-212; Update List Source.', 
                                   'Description of Change': None, 
                                   'Date Issued': '2020-11-24', 
                                   'Source': 'Online Browsing Platform (OBP) - https://www.iso.org/obp/ui/#iso:code:3166:SI.'}, 
                                  {'Change': 'Add 1 commune SI-212; update List Source.', 
                                   'Description of Change': None, 
...
#4.)
        export_iso3166_2(alpha_codes=test_alpha_lu_mk_om, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, history=True) #Luxembourg, North Macedonia, Oman

        #open exported json & csv
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_LU,MK,OM.json')) as output_json:
            test_iso3166_2_lu_mk_om_json = json.load(output_json)

        #LU-CA - Capellen
        expected_lu_ca_history = [{'Change': 'Addition of cantons LU-CA, LU-CL, LU-DI, LU-EC, LU-ES, LU-GR, LU-LU, LU-ME, LU-RD, LU-RM, LU-VD, LU-WI; update List Source.', 
                                   'Description of Change': None, 
        ...
        #OM-ZU - Z̧ufār
        expected_om_zu_history = [{'Change': 'Change of subdivision category from region to governate for OM-BA, OM-DA, OM-SH, OM-WU, OM-ZA; change of subdivision code from OM-BA to OM-BJ, from OM-SH to OM-SJ; change of spelling of OM-BJ, OM-SJ; addition of governorates OM-BS, OM-SS; addition of local variations of OM-MA, OM-ZU; update List Source.', 
                                   'Description of Change': None, 
                                   'Date Issued': '2015-11-27', 
                                   'Source': 'Online Browsing Platform (OBP) - https://www.iso.org/obp/ui/#iso:code:3166:OM.'}, 
                                  {'Change': 'Subdivisions added: OM-BU Al Buraymī. Codes: OM-JA Al Janūbīyah -> OM-ZU Z̧ufār.', 
...
#1.)
        export_iso3166_2(alpha_codes=test_alpha_at_city_data, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, state_city_data=True) #Austria 

        #open exported json & csv
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_AT.json')) as output_json:
            test_iso3166_2_at_json = json.load(output_json)
        
        expected_at_1_city_names = ['Andau', 'Antau', 'Apetlon', 'Bad Sauerbrunn', 'Bad Tatzmannsdorf', 'Badersdorf', 'Bernstein', 'Bocksdorf', 'Breitenbrunn', 'Bruckneudorf', 
        ...
        observed_at_1_city_names = [item["name"] for item in test_iso3166_2_at_json["AT"]["AT-1"]["cities"]]

        #iterate through subset of city names for subdivision 
        for city in expected_at_1_city_names:
            self.assertIn(city, observed_at_1_city_names, f"City {city} not found in list of cities for AT-1 subdivision.")
#2.)
        export_iso3166_2(alpha_codes=test_alpha_km_city_data, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, state_city_data=True) #Comoros 
        ...
        expected_km_g_city_names = ['Bahani', 'Bambadjani', 'Bouni', 'Chezani', 'Chindini', 'Chouani', 'Dembéni', 'Douniani', 'Dzahadjou', 'Foumbouni', 'Hantsindzi', 
        ...
        observed_km_g_city_names = [item["name"] for item in test_iso3166_2_km_json["KM"]["KM-G"]["cities"]]

        #iterate through subset of city names for subdivision 
        for city in expected_km_g_city_names:
            self.assertIn(city, observed_km_g_city_names, f"City {city} not found in list of cities for KM-G (Andjazîdja) subdivision.")
#3.)
        export_iso3166_2(alpha_codes=test_alpha_mz_city_data, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, state_city_data=True) #Mozambique 

        #open exported json & csv
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_MZ.json')) as output_json:
            test_iso3166_2_mz_json = json.load(output_json)

        expected_mz_mpm_city_names = ['KaTembe', 'Maputo']  #Maputo
        observed_mz_mpm_city_names = [item["name"] for item in test_iso3166_2_mz_json["MZ"]["MZ-MPM"]["cities"]]
        #iterate through subset of city names for subdivision 
        for city in expected_mz_mpm_city_names:
            self.assertIn(city, observed_mz_mpm_city_names, f"City {city} not found in list of cities for MZ-MPM subdivision.")
#4.)
        export_iso3166_2(alpha_codes=test_alpha_pg_ro_city_data, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=1, state_city_data=True) #Papa New Guinea, Romania 

        #open exported json & csv
        with open(os.path.join(self.test_output_dir, f'{self.test_output_filename}_PG,RO.json')) as output_json:
            test_iso3166_2_pg_ro_json = json.load(output_json)

        expected_pg_ro_city_names = {"PG-NIK": ['Kavieng', 'Namatanai'], "RO-DB": ['Adânca', 'Aninoasa', 'Bezdead', 'Bilciureşti', 'Braniştea', 'Brezoaele', 'Brezoaia', 'Broșteni', 
        ...
        observed_pg_nik_city_names = [item["name"] for item in test_iso3166_2_pg_ro_json["PG"]["PG-NIK"]["cities"]]
        observed_ro_db_city_names = [item["name"] for item in test_iso3166_2_pg_ro_json["RO"]["RO-DB"]["cities"]]

        #iterate through subset of city names per subdivision 
        for city in expected_pg_ro_city_names["PG-NIK"]:
            self.assertIn(city, observed_pg_nik_city_names, f"City {city} not found in list of cities for PG-NIK subdivision.")
        for city in expected_pg_ro_city_names["RO-DB"]:
            self.assertIn(city, observed_ro_db_city_names, f"City {city} not found in list of cities for RO-DB subdivision.")
...
#1.)
        export_iso3166_2(alpha_codes_range=test_alpha_et_fj, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=0, export_xml=0, 
            save_each_iteration=True) #Ethiopia - Fiji

        self.assertTrue(os.path.isfile(os.path.join(self.test_output_dir, f'{self.test_output_filename}_ET.json')), 
            f"Expected output JSON file to exist in folder: {os.path.join(self.test_output_dir, f'{self.test_output_filename}_ET.json')}.")
        self.assertTrue(os.path.isfile(os.path.join(self.test_output_dir, f'{self.test_output_filename}_FI.json')), 
            f"Expected output JSON file to exist in folder: {os.path.join(self.test_output_dir, f'{self.test_output_filename}_FI.json')}.")
        self.assertTrue(os.path.isfile(os.path.join(self.test_output_dir, f'{self.test_output_filename}_FJ.json')), 
            f"Expected output JSON file to exist in folder: {os.path.join(self.test_output_dir, f'{self.test_output_filename}_FJ.json')}.")
#2.)
        export_iso3166_2(alpha_codes_range=test_alpha_lv_ma, export_folder=self.test_output_dir, export_filename=self.test_output_filename, verbose=0, export_csv=0, export_xml=0, 
            save_each_iteration=True) #Latvia - Morocco

        self.assertTrue(os.path.isfile(os.path.join(self.test_output_dir, f'{self.test_output_filename}_LV.json')), 
            f"Expected output JSON file to exist in folder: {os.path.join(self.test_output_dir, f'{self.test_output_filename}_LV.json')}.")
        self.assertTrue(os.path.isfile(os.path.join(self.test_output_dir, f'{self.test_output_filename}_LY.json')), 
            f"Expected output JSON file to exist in folder: {os.path.join(self.test_output_dir, f'{self.test_output_filename}_LY.json')}.")
        self.assertTrue(os.path.isfile(os.path.join(self.test_output_dir, f'{self.test_output_filename}_MA.json')), 
            f"Expected output JSON file to exist in folder: {os.path.join(self.test_output_dir, f'{self.test_output_filename}_MA.json')}.")
